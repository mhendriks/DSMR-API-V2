...
#define MAX_MONITORED_CLIENTS 2
#endif
  ...
  void acceptClients() {
    for (uint8_t i = 0; i < MAX_MONITORED_CLIENTS; i++) {
		if (!connectedClients[i]) { 
			...
			// aanpassing: sta maar 1 telnet verbinding toe; bij 2e verbinding verbreek de eerste 
			if (connectedClients[i]) { 
				connectedClients[(i+1)%MAX_MONITORED_CLIENTS].println(F("Andere Telnet sessie gestart -> Deze connectie verbroken"));connectedClients[(i+1)%MAX_MONITORED_CLIENTS].stop();
			} //alleen deze if is extra
		}
	} //for
  }//acceptClients
};
